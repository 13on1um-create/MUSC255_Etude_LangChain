...
if not openai_api_key.startswith("sk-"):
    ...
    with st.form("my_form"):
        question = st.text_area(
            "Enter question:",
            "How do I improve the tone quality of my voice?",
        )
        k = st.text_area(
        ...
        if not k.isdigit():
            ...
            k = int(k)
            if k < 1 or k > 10:
                st.warning('Please enter a number between 1-7.', icon='⚠')
            elif submitted:
                ...
                st.info('The following are the text chunks fetched from _The Etude_:')
